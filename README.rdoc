...
Planetoid is a lightweight planet, for small communities who want to aggregate their feeds in one single place.
...
Also, Planetoid uses the awesome Feedzirra gem to fetch the feeds. You'll need to install this gem too, so you can find more about it at http://github.com/pauldix/feedzirra

= Installation notes

Just like a normal rails app:
...
From the initial install, you'll be prompted to create your first users and feeds. When you access these admin sections, you'll be asked for the admin login/password. Default login and passwords are on the app/controllers/application_controller.rb file, CHANGE THEM BEFORE GOING INTO PRODUCTION!

I'm planning of a better admin interface, but for now if you want to enter into the admin section, just go to http://localhost:3000/users
...
* Move config variables to a file
